...
//		if (empty($result['SALON_CONFIG_SEND_MAIL_TEXT_USER']) ) $result['SALON_CONFIG_SEND_MAIL_TEXT_USER'] = __('Mr/Ms {X-TO_NAME} Thank you for registration .your User_id is %s,your initial password is %s',SL_DOMAIN);
		...
		if (!isset($result['SALON_CONFIG_WEEK_FIRST']) ) $result['SALON_CONFIG_WEEK_FIRST'] =  Salon_Week::MONDAY;
		...
		if (empty($result['SALON_CONFIG_USE_PC_MOBILE']) ) $result['SALON_CONFIG_USE_PC_MOBILE'] = Salon_Config::PC_MOBILE_USE;
 		...
 		if (!isset($result['SALON_CONFIG_LINK_START_MINUTE']) ) $result['SALON_CONFIG_LINK_START_MINUTE'] = 5;
	...
	public function getTargetPositionData($position_cd){
		global $wpdb;
		$sql = $wpdb->prepare(' SELECT * FROM '.$wpdb->prefix.'salon_position '
				.' where delete_flg <> %d'
				.' AND position_cd = %d '
...
// 		return count($rows);
	...
	public function countReservationAnyone($branch_cd ,$in_time,$out_time
			,$reservation_cd,$item_cds,$branch_duplicate_cnt) {
		global $wpdb;

		$sql = $wpdb->prepare(' SELECT in_items ,staff_cd,duplicate_cnt'.
				' ,length(in_items) as len '.
				' FROM '.$wpdb->prefix.'salon_staff '.
				' WHERE  delete_flg <> '.Salon_Reservation_Status::DELETED.
				' AND branch_cd = %d  '
				,$branch_cd);
		...
		$sql .= " ORDER BY len,in_items,staff_cd ";
		...
		if (count($result_staff) == 0) {
			throw new Exception(Salon_Component::getMsg('E901',"No Staff data:".__LINE__) );
		}

		$edit_staff = array();
		$where_in_staff = array();
		foreach ($result_staff as $k1 => $d1 ) {
			$where_in_staff[] = $d1['staff_cd'];
			$edit_staff[$d1['staff_cd']] = array();
			$edit_staff[$d1['staff_cd']]['current_cnt'] = 0;
			$edit_staff[$d1['staff_cd']]['max_cnt'] = 1 + $d1['duplicate_cnt'];
			$edit_staff[$d1['staff_cd']]['in_items'] = $d1['in_items'];
		}
		$where = "";
		if (!empty($reservation_cd) ) $where = $wpdb->prepare('AND reservation_cd <> %d ',$reservation_cd);
		...
		if ( $wpdb->query($sql) === false ) {
			...
			$result_reservation = $wpdb->get_results($sql,ARRAY_A);
		}
		foreach ($result_reservation as $k1 => $d1) {
			if ($d1['staff_cd'] != Salon_Default::ANYONE) {
				$edit_staff[$d1['staff_cd']]['reservation'][] = $d1;
				$set_key = count($edit_staff[$d1['staff_cd']]['reservation']);
				$edit_staff[$d1['staff_cd']]['key_from'][$set_key] = $d1['edit_from'];
				$edit_staff[$d1['staff_cd']]['key_to'][$set_key] = $d1['edit_to'];
			}
		}
		//出退勤情報
		//スタッフの勤務時間で、休み部分があれば
		...
		' SELECT  '
			...
			.' ,out_time '
			.' ,working_cds '
			.',DATE_FORMAT(in_time,"%%Y%%m%%d%%H%%i") as edit_in_time '
			.',DATE_FORMAT(out_time,"%%Y%%m%%d%%H%%i") as edit_out_time '
			.' FROM '.$wpdb->prefix.'salon_working '
			.'   WHERE '
			.'         %s <= out_time '
			.'     AND in_time <= %s '
			.'     AND staff_cd in  ('.implode(',',$where_in_staff).')'
			.'     AND delete_flg <> %d '
			.'   ORDER BY staff_cd,in_time,out_time ';

		$sql = $wpdb->prepare($sql	,
				$in_time,$out_time,Salon_Reservation_Status::DELETED);
		if ( $wpdb->query($sql) === false ) {
			$this->_dbAccessAbnormalEnd();
		}
		else {
			$result_working = $wpdb->get_results($sql,ARRAY_A);
		}
		if ( 0 < count($result_working)  ) {

			$reservation_in_time = new DateTime($in_time);
			$reservation_out_time = new DateTime($out_time);
			foreach($result_working as $k1 => $d1) {
				//休みの場合は該当時間を予約にする
				//休みのコードは2のみが設定される前提
				if ($d1['working_cds'] == Salon_Working::DAY_OFF ) {

					$edit_staff[$d1['staff_cd']]['reservation'][] =
						array (
							"staff_cd" => $d1['staff_cd']
							,"time_from" => $d1['in_time']
							,"time_to" => $d1['out_time']
							,"edit_from" => $d1['edit_in_time']
							,"edit_to" => $d1['edit_out_time']
							,"item_cds" => ""
						);
					$set_key = count($edit_staff[$d1['staff_cd']]['reservation']);
					$edit_staff[$d1['staff_cd']]['key_from'][$set_key] = $d1['edit_in_time'];
					$edit_staff[$d1['staff_cd']]['key_to'][$set_key] = $d1['edit_out_time'];

				}
				else {
					//fromが予約時間開始の後ろ
					//→fromまでは休み=そこを予約にする。
					$working_in_time = new DateTime($d1['in_time']);
					if ($reservation_in_time < $working_in_time) {
						...
						array (
							...
							,"time_to" => $d1['in_time']
							,"edit_from" =>
							$reservation_in_time->format("YmdHi")
							,"edit_to" => $d1['edit_in_time']
							,"item_cds" => ""
						);
						$set_key = count($edit_staff[$d1['staff_cd']]['reservation']);
						$edit_staff[$d1['staff_cd']]['key_from'][$set_key] = $reservation_in_time->format("YmdHi");
						$edit_staff[$d1['staff_cd']]['key_to'][$set_key] = $d1['edit_in_time'];


					}
					//toが予約時間終了の前
					//→toから休み=そこを予約にする。
					$working_out_time = new DateTime($d1['out_time']);
					if ($working_out_time < $reservation_out_time ) {

						$edit_staff[$d1['staff_cd']]['reservation'][] =
						array (
							"staff_cd" => $d1['staff_cd']
							,"time_from" => $d1['out_time']
							,"time_to" => $out_time
							,"edit_from" => $d1['edit_out_time']
							,"edit_to" =>
							$reservation_out_time->format("YmdHi")
							,"item_cds" => ""
						);
						$set_key = count($edit_staff[$d1['staff_cd']]['reservation']);

						$edit_staff[$d1['staff_cd']]['key_from'][$set_key] = $d1['edit_out_time'];
						$edit_staff[$d1['staff_cd']]['key_to'][$set_key] = $reservation_out_time->format("YmdHi");
					}
				}
			}
		}
		...
		foreach ($edit_staff as $k1 => $d1) {
			if (isset($d1['key_from'])	) {
				...
				//実際に埋まっている時間を設定する
				$edit_staff[$k1]['current_cnt'] =
					$this->_calcReservationHierachcal($d1['reservation']);
			}
		}
		//指名なしの予約をどっかのスタッフに割り当てる
		$anyone_cnt = 0;
		$array_check_items = array();
		foreach ($result_reservation as $k1 => $d1 ) {
			if ($d1['staff_cd'] == Salon_Default::ANYONE) {
				$anyone_cnt++;
				//指名なしで指定されているメニューを設定しとく
				$array_check_items[] = array(false,explode(',',$d1['item_cds']));
			}
		}
		if ( 0 < $anyone_cnt) {
			$add_cnt = 0;
			...
			foreach ($edit_staff as $k1 => $d1) {
				...
				foreach ($array_check_items  as $k2 => $d2) {
					$is_Ok = false;
					//チェック済み？
					if ($d2[0] == false) {
						foreach($d2[1] as $d3) {
							if (in_array($d3,$base_items) === false ) {
								$is_Ok = false;
								break;
							}
							else {
					...
					if ($is_Ok){
						$array_check_items[$k2][0] = true;
						break;
					}
				}
				if ($is_Ok)	 {
					$add_valid_cnt = $d1['max_cnt'] - $d1['current_cnt'];
					$add_cnt = $add_valid_cnt;
					if ($anyone_cnt <= $add_valid_cnt) {
						$add_cnt = $anyone_cnt;
					}
					$edit_staff[$k1]['current_cnt'] += $add_cnt;
					$anyone_cnt -= $add_cnt;
				}
				if ($anyone_cnt == 0) {
					break;
			...
			//指名なしをすべてどこかに埋められない場合エラー
			//ただし、支店の重複数分はOK
			if (0 < $anyone_cnt - $branch_duplicate_cnt) {
				return false;
			}
		}
		//支店単位の重複可能数があるので、
		//現状の余裕数をチェックする。
		if (0 < $branch_duplicate_cnt) {
			$valid_cnt = $branch_duplicate_cnt - $anyone_cnt;
			foreach ($edit_staff as $k1 => $d1) {
				$valid_cnt += $d1['max_cnt'] - $d1['current_cnt'];
			}
			if (0 < $valid_cnt) {
				return true;
			}
...
// 	public function countReservationAnyoneOld($branch_cd ,$in_time,$out_time
// 			,$reservation_cd,$item_cds,$branch_duplicate_cnt ) {
// 		global $wpdb;
...
// 		}
// 		$staff_working = array();
// 		foreach ($where_in_staff as $d1 ) {
// 			$staff_working[$d1] = array();
// 		}
// 		if ( 0 < count($result_working)  ) {
...
// 				//休みのコードは2のみが設定される前提
// 				if ($d1['working_cds'] == Salon_Working::DAY_OFF ) {
// 					$staff_working[$d1['staff_cd']][$d1['edit_in_time']] =
// 					array (
// 							"staff_cd" => $d1['staff_cd']
// 							,"time_from" => $d1['in_time']
// 							,"time_to" => $d1['out_time']
...
// 					$working_in_time = new DateTime($d1['in_time']);
// 					if ($reservation_in_time < $working_in_time) {
// 						$staff_working[$d1['staff_cd']][$reservation_in_time->format("YmdHi")] =
// 						array (
// 								"staff_cd" => $d1['staff_cd']
// 								,"time_from" => $in_time
// 								,"time_to" => $d1['in_time']
...
// 					$working_out_time = new DateTime($d1['out_time']);
// 					if ($working_out_time < $reservation_out_time ) {
// 						$staff_working[$d1['staff_cd']][$d1['edit_out_time']] =
// 						array (
// 								"staff_cd" => $d1['staff_cd']
// 								,"time_from" => $d1['out_time']
// 								,"time_to" => $out_time
...
// 			$result = $wpdb->get_results($sql,ARRAY_A);
// 		}
// 		if (count($result) == 0 && count($result_working) == 0) {
// 			return true;
// 		}
...
// 		//予約のないスタッフは空データを設定
// 		foreach ($where_in_staff as $d1 ) {
// 			$staff_result[$d1] = array();
// 		}
// 		for ($i = 0 ; $i < $max_loop ; $i++ ){
// 			$staff_result[$result[$i]["staff_cd"]][] =
// 				$result[$i];
// 		}
// 		foreach ($where_in_staff as $d1 ) {
// 			$new_result = $this->_editWorkingStaffData(
// 					$staff_result[$d1]
// 					,$staff_working[$d1]);
// 			//データがない=予約もない、出退勤の情報もない
// 			if (count($new_result) == 0) {

// 			}
// 			else {
// 				$cnt = $this->_calcReservationHierachcal($new_result);
// 				$edit_staff[$d1]['current_cnt'] = $cnt;
// 			}
// 		}
// 		//指名なしにすでに登録がある場合の考慮
// 		$anyone_cnt = 0;
// 		if (isset($staff_result[Salon_Default::ANYONE])) {
// 			$anyone_cnt = count($staff_result[Salon_Default::ANYONE]);
// 		}

// 		if ( 0 < $anyone_cnt) {
// 			//最初に指名なしの指定メニューを取得する
// 			$array_check_items = array();
// 			foreach($staff_result[Salon_Default::ANYONE] as $k1 => $d1) {
// 				$array_check_items[] = array(false,explode(',',$d1['item_cds']));
// 			}
// 			foreach ($edit_staff as $k1 => $d1) {
// 				//スタッフのキャパに余裕があるところに
...
// // 			if ($save_staff_cd != $result[$i]['staff_cd']) {
// // 				$new_result = $this->_editWorkingStaffData(
// // 						$staff_result,$staff_working[$save_staff_cd]);
...
// 	}
	...
	private function _editWorkingStaffData($reservation_datas,$working_datas){
		$new_result = array();
		//reservation_data 該当範囲のスタッフ単位の予約 from順
		//
		//staff単位にworkingデータをはさみこむ
		$max_reservation = count($reservation_datas);
		$max_working = count($working_datas);
		$i_reservation = 0;
		$i_working = 0;
		if ($max_reservation == 0 && $max_working == 0 ) {
			return $new_result;
		}
		elseif ($max_reservation == 0 ) {
			return $working_datas;
		}
		elseif ($max_working == 0 ) {
			return $reservation_datas;
		}
		$working_datas_temp = array();
		foreach ($working_datas as $k1 => $d1) {
			$working_datas_temp[] = $d1;
		}
		for (;;) {
			if ($reservation_datas[$i_reservation]["edit_from"] <=
					$working_datas_temp[$i_working]["edit_from"]) {
				$new_result[]= $reservation_datas[$i_reservation];
				if ($i_reservation < $max_reservation) {
					$i_reservation++;
				}
				else {
					if ($i_reservation == $max_result
							&& $i_working == $max_working ) {
								break;
							}
				}
			}
			else {
				$new_result[] = $working_datas_temp[$i_working];
				if ($i_working < $max_working) {
					$i_working++;
				}
				else {
					if ($i_reservation == $max_result
...
// 				}
	...
	public function getLinkData ($reservation_cd ) {
		global $wpdb;
	...
	public function updateTableForLink ($table_data){
		...
		$set_data_temp = array(
				...
				$table_data['event_time'],
				$table_data['arguments'],
				$table_data['delete_flg'],
				date_i18n('Y-m-d H:i:s')	);

		$set_data_temp[] = $table_data['reservation_cd'];
		$where_string = ' reservation_cd = %d ';
		$sts = $this->updateSql('salon_link ',$set_string,$where_string,$set_data_temp);
		if ($sts === false ) {
			$this->_dbAccessAbnormalEnd();
		}
