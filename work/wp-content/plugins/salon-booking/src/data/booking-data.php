...
class Booking_Data extends Salon_Data {
	...
	const LINK_TABLE_NAME = 'salon_link';
...
// 		$to_date->add(new DateInterval("P".$dayDiff."DT".$minDiff."M"));
	...
	public function getTargetOldReservation($reservation_cd){
		global $wpdb;

		$sql = 	$wpdb->prepare(
				' SELECT '.
				' DATE_FORMAT(time_from,"%%Y%%m%%d%%H%%i") as time_from,'.
				' DATE_FORMAT(time_to,"%%Y%%m%%d%%H%%i") as time_to '.
				' FROM '.$wpdb->prefix.'salon_reservation '.
				'   WHERE  '.
				'     reservation_cd = %d ',
				$reservation_cd
	...
	public function insertTableForLink ($table_data){

		$sts = $this->insertSql(self::LINK_TABLE_NAME
				, $table_data,'%d,%s,%d,%d,%s,%s,%s,%d,%s');
		if ($sts === false ) {
			$this->_dbAccessAbnormalEnd();
		}
		return true;
