...
<?php 		//24時間超えの場合
			...
			if ($this->config_datas['SALON_CONFIG_WEEK_FIRST'] == Salon_Week::SUNDAY) {
				echo 'scheduler.config.start_on_monday = false;';
			}
?>
	...
	//				$tmp = preg_replace("/(width|height)(=\\\'\d+\\\')/","$1=\'50\'",$d1['photo']);
			...
			foreach ($this->staff_datas as $k1 => $d1 ) {
				...
				if (!empty($d1['in_items'])) {
					$menus_array = explode(',',$d1['in_items']);
					foreach ($menus_array as $k2 => $d2 ) {
						if (! array_key_exists($d2,$menus_staff) ) {
							$menus_staff[$d2] = array();
						}
						$menus_staff[$d2][$d1['staff_cd']] = "";
					}
				}
				$reserve_possible_cnt += 1+$d1['duplicate_cnt'];
				echo 'staff_names['.$d1['staff_cd'].'] = "'.$d1['name'].'";';
			}
			foreach ($menus_staff as $k1 => $d1 ) {
				$staffCanOperate = array();
				foreach ($d1 as $k2 => $d2 ) {
					$staffCanOperate[] = $k2;
				}
				echo 'menus_staff['.$k1.'] = "'.implode(',',$staffCanOperate).'";';
			}
	...
	<?php //section_autoheightはスタッフの人数が多い場合はfalseにする
		...
		<?php	endif; ?>
			...
			scheduler.templates.event_class=function(start,end,event){
				var set_class = "";
				if (event.status == <?php echo Salon_Reservation_Status::TEMPORARY; ?> ) {
					set_class = "user_temporary";
				}
				if (event.edit_flg == <?php  echo Salon_Edit::NG; ?> ) {
...
?>
			...
			$j("#sl_item_cds input[type=<?php $this->echo_menu_type(); ?>]").click(function(){
				...
				<?php
					foreach($this->staff_datas as $k1 => $d1 ) {
						echo 'can_operate_staff['.$d1['staff_cd'].'] = 0;';
					}
				?>
...
// 						minute += +$j(this).next().next().val();
				...
				if (0 < checked_menu_cnt ) {
					<?php //delete staff select tag ?>
					$j("select#sl_staff_cd").children().remove();
					for ( var set_staff_cd in can_operate_staff ) {
						<?php //is not same value, this staff can not operate checked menus ?>
					...
					<?php //more then 2 staff members, first position is "anyone" ?>
					<?php if ($this->_is_noPreference() ) : ?>
					if ( 1 < set_staff_cnt ) {
						$j("select#sl_staff_cd").prepend("<option value='"
								+ "-1" + "'><?php _e('Anyone',SL_DOMAIN); ?></option>");
					...
					<?php //no check menu, so every staff members show?>
					$j("select#sl_staff_cd").children().remove();
					<?php if ($this->_is_noPreference() ) : ?>
						$j("select#sl_staff_cd").prepend("<option value='"
								+ "-1" + "'><?php _e('Anyone',SL_DOMAIN); ?></option>");
				...
				<?php //set selected staff member before this process ?>
...
?>
	...
	<style>
	<?php if ($this->_is_noPreference()) : ?>
.sl_staff_<?php echo Salon_Default::ANYONE; ?>
{
		background-color:<?php echo $this->config_datas['SALON_CONFIG_PC_EVENT_COLOR'];?> !important;
}
	<?php endif;?>
	<?php foreach ($this->staff_datas as $k1 => $d1 ) : ?>

.sl_staff_<?php echo $d1['staff_cd']; ?>
{

		<?php if ((!empty($d1['notes']) && substr($d1['notes'],0,1) == '#')) : ?>
			background-color:<?php echo $d1['notes'];?> !important;
		<?php else :?>
			background-color:<?php echo $this->config_datas['SALON_CONFIG_PC_EVENT_COLOR'];?> !important;
