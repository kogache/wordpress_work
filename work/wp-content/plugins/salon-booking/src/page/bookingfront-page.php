...
class BookingFront_Page extends BookingCommon_Page {
	...
	public function set_config_datas($config_datas) {
		...
		$edit_datepicker = Salon_Component::computeDate($config_datas['SALON_CONFIG_AFTER_DAY']-1);
		$this->datepicker_max_day = substr($edit_datepicker,0,4).','.(intval(substr($edit_datepicker,5,2))-1).','.(intval(substr($edit_datepicker,8,2)));
