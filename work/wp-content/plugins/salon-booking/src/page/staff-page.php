...
// 		}
	...
	public function show_page() {
		if ($this->isSalonAdmin() ) {
			$this->branch_column = 4;
			$this->position_column = 5;
			$this->set_items = array('first_name','last_name','branch_cd','position_cd','zip','address','tel','mobile','mail','user_login','remark','employed_day','leaved_day','duplicate_cnt_staff','item_cds_set','memo','color_staff');
		}
		else {
			$this->branch_column = 3;
			$this->position_column = 4;
			$this->set_items = array('first_name','last_name','position_cd','zip','address','tel','mobile','mail','user_login','remark','employed_day','leaved_day','duplicate_cnt_staff','item_cds_set','memo','color_staff');
		}
?>
...
//						,palettes: false // show a palette of basic colors beneath the square.
				,palettes:['<?php echo Salon_Color::PC_BACK_STAFF_PALLET1; ?>'
				, '<?php echo Salon_Color::PC_BACK_STAFF_PALLET2; ?>'
				, '<?php echo Salon_Color::PC_BACK_STAFF_PALLET3; ?>'
				, '<?php echo Salon_Color::PC_BACK_STAFF_PALLET4; ?>'
				, '<?php echo Salon_Color::PC_BACK_STAFF_PALLET5; ?>'
				, '<?php echo Salon_Color::PC_BACK_STAFF_PALLET6; ?>']
			});
...
<?php //taregt_colはtdが前提 ?>
		function fnSelectRow(target_col) {
			...
			$j("#staff_color").iris("color","<?php echo $this->config_datas['SALON_CONFIG_PC_EVENT_COLOR']; ?>");
			if (setData['aoData'][position[0]]['_aData']['notes']) {
				$j("#staff_color").iris("color",setData['aoData'][position[0]]['_aData']['notes']);
			}
		}
...
<?php endif; ?>
		...
		<div  class="sl_color_warp" >
			<input id="staff_color" type="text" class="color-picker" value="" >
		</div>
