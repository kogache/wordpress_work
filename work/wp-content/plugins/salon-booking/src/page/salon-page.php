...
//		$echo_data .=   '<option value="-1" >'.__('no setting',SL_DOMAIN).'</option>';
		...
		if ($close_time == "2359") {
			$echo_data .= '<option value="24:00" >24:00</option>';
		}
		$echo_data .= '</select>';
...
EOT;
	...
	static function echoTimeStepSelect($id,$is_noEcho = false) {
		...
		$datas = apply_filters('salon_change_TimeStep_Select',$datas);
	...
	static function echoMinuteSelect($id,$is_noEcho = false) {
		...
		$datas = apply_filters('salon_change_Minute_Select',$datas);
...
// 		$max->add(new DateInterval("P".$this->config_datas['SALON_CONFIG_AFTER_DAY']."D"));
		...
		//1月余分に読んでおく
		$max->modify("+".($this->config_datas['SALON_CONFIG_AFTER_DAY']+31)." day");
		...
		echo '
			scheduler.locale={
				date: {
					...
					day_short:['.__('"Su","Mo","Tu","We","Th","Fr","Sa"',SL_DOMAIN).'],
					dayNamesMin: ['.__('"Su","Mo","Tu","We","Th","Fr","Sa"',SL_DOMAIN).'],
				},
				labels:{
	...
	static function set_datepickerDefault($is_maxToday = false,$is_all = false){
		...
		echo
			'$j.datepicker.setDefaults({
					...
					dayNamesShort: ['.__('"Su","Mo","Tu","We","Th","Fr","Sa"',SL_DOMAIN).'],
					dayNamesMin: ['.__('"Su","Mo","Tu","We","Th","Fr","Sa"',SL_DOMAIN).'],
					...
					if (sp_date) {
						if (sp_date.type == '.$tmp_status.' ) {
							...
							&& (sp_date.close == "'.$close_time.'")) {
								...
								result =  [true, "slm-date-holiday4", sp_date.title];
							}
						}
					 }
					 else {
...
// 		 ,'tips' => __('Now plural shops select only.',SL_DOMAIN));
		...
		$item_contents['send_mail_text'] =array('id'=>'send_mail_text'
		 ,'class' => array()
		 ,'check' => array( 'lenmax500')
		 ,'label' => '17.'.__('The Content of the Mail to Confirming Notice to the Client',SL_DOMAIN)
		 ,'tips' => __('within 500 charctors. {X-TO_NAME} in the content replace customer name',SL_DOMAIN));
		...
		$item_contents['confirm_week'] =array('id'=>'config_week_monday'
		,'class'	=>array()
		,'check' => array()
		,'label' => '19.'.__('First day',SL_DOMAIN)
		,'tips' => __('Please check first day of the week at Reservation Screen.',SL_DOMAIN));
		...
		$item_contents['color_staff'] =array('id'=>'staff_color'
				,'class'	=>array()
				,'check' => array()
				,'label' => __('Staff color',SL_DOMAIN)
				,'tips' => __('Staff color on \"Reservtion\"',SL_DOMAIN));
	...
	public function serverCheckBooking($items , &$msg , &$focus_field) {
		...
		foreach ($items as $d1) {
			...
			foreach ($item_contents[$d1]['check'] as $d2 ) {
				...
				if ($focus_field == "" && $before != count($err_msg)) {
					$focus_field = $id;
				}
			}
		}
...
// 						//24時を終了時間にしている店舗で、24時まで働く場合
// 						if ((substr($d2['in_time'],0,8) != substr($d2['out_time'],0,8)
// 							&& (substr($d2['out_time'],-4) = "0000") ) {
