...
//							'     AND staff_cd = %d ';
				...
				if ($cnt > $result[0]['duplicate_cnt'] ) {
					throw new Exception(self::getMsg('W002',array(__('staff',SL_DOMAIN), $result[0]['duplicate_cnt']+1)),__LINE__);
				}
...
//[2014/07/15]Ver 1.4.3
			...
			//指名なしの場合は、店とスタッフの重複数をチェックする。
			//出勤間のみ予約可のパターンは、指名なしを選択できないので
			//考慮する必要がない
			//=>下の支店単位のチェックでひっかかるはずだからここではいらない
// 			else {
...
//			}
			...
			if ($cnt >= $possible_cnt ) {
				throw new Exception(self::getMsg('W002',array(__('branch',SL_DOMAIN), $possible_cnt)),__LINE__);
			}
			//[2017/06/14]指名なしの場合に選択されたメニューを開いているスタッフが扱えるか
			if  ($set_data['staff_cd'] ==  Salon_Default::NO_PREFERENCE ) {
				//[2017/09/21]指名なしの重複数で、勤務時間外の場合の考慮を追加
				if ($datas->countReservationAnyone($set_data['branch_cd']
						,$set_data['time_from'],$set_data['time_to']
						,$reservation_cd,$set_data['item_cds']
						,$result_branch[0]['duplicate_cnt']) === false ) {
							throw new Exception(self::getMsg('W005'),__LINE__);
						}
			}
			...
			if ($datas->isPromotion() ) {
				...
				if (isset($set_data['coupon']) && !empty($set_data['coupon']) ) {
					...
					if (!$datas->checkCustomerPromotion($set_data,$result_promotion[0],$add_char,$reservation_cd  ) ) {
						throw new Exception(self::getMsg('E302',$add_char),_LINE__);
					}
				}
			}
	...
	static function getMsg($err_cd, $add_char = '') {
		...
		switch ($err_cd) {
			...
			case 'E921':
				...
				break;
			case 'E931':
				$err_msg = sprintf(__("Link error[%s]",SL_DOMAIN),$add_char);
				break;
			case 'W001':
