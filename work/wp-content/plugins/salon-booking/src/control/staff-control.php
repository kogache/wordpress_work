	...
	public function do_action() {
		...
		elseif ($this->action_class == 'Staff_Edit' ) {
			...
			if ( $_POST['type'] == 'deleted' && $this->datas->isCompleteDelete() ) {
				$reRead = $this->comp->editInitData($this->datas->getStaffDataByUser($res['user_login']),$branch_cd);
			}
			else {
				$reRead = $this->comp->editInitData($this->datas->getStaffDataByStaffcd($res['staff_cd']),$branch_cd);
			}
