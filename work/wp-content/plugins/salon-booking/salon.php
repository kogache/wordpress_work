...
if ( ! defined( 'SALON_FOR_LINK' ) ) {
	define( 'SALON_FOR_LINK', false);
}
if ( ! defined( 'SALON_FOR_LINK_LOG' ) ) {
	define( 'SALON_FOR_LINK_LOG', false);
}
...
//		add_action('init', array( &$this, 'init_session_start'));
		...
		add_action('wp_ajax_slcheckconfig', array( &$this,'salon_booking_check_config'));
...
//			add_action('admin_head',  array( &$this,'my_admin_head'));
		...
		if (SALON_FOR_LINK ) {
//  			add_action( 'wp_ajax_sllink',  array( &$this,'edit_link') );
			require_once(SL_PLUGIN_SRC_DIR . 'comp/linkFunctionHotpepper-component.php');
			add_action('wp_ajax_sllink', array( &$this,'edit_link'));
		}
...
//[Ver1.5.1]
			...
			if (SALON_FOR_LINK ) {
				$my_admin_page = add_submenu_page( $this->maintenance, __('Link',SL_DOMAIN), __('Link',SL_DOMAIN), 'edit_posts', 'edit_link' , array( &$this, 'edit_link' ) );
			}
...
//[Ver1.5.1]
	...
	public function edit_link() {
		require_once( SL_PLUGIN_SRC_DIR.'/control/link-control.php' );
		$control = new Link_Control();
		$control->exec();
	}
...
//[Ver1.5.1]
		...
		if (SALON_FOR_LINK ) {
		$wpdb->query("	CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."salon_link (
								`reservation_cd`	INT not null,
								`link_cd`		char (30)  not null,
								`service_cd`	INT not null,
								`link_status`		INT,
								`information`	TEXT,
								`remark`		TEXT,
								`memo`			TEXT,
								...
								`update_time`	DATETIME,
							PRIMARY KEY  (`reservation_cd`),
							INDEX idx_link(link_cd)
						) ".$charset_collate);
// 		$wpdb->query("	CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."salon_service (
// 								`service_cd`	INT not null,
// 								`name`			VARCHAR(40),
// 								`logon_id`		VARCHAR(40),
// 								`password`		VARCHAR(40),
// 								`mail_identifier`	TEXT,
// 								`toServiceRemark`	varchar(256),
// 								`remark`		TEXT,
// 								`memo`			TEXT,
// 								`notes`			TEXT,
// 								`delete_flg`	INT default 0,
